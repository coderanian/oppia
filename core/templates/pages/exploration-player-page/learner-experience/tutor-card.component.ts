...
export class TutorCardComponent {
  ...
  setRandomAvatarImage(): void {
    let newUrl = '';
    do {
      const index = Math.floor(Math.random() * AppConstants.AVATAR_IMAGE_COUNT) + 1;
      newUrl = this.urlInterpolationService.getStaticCopyrightedImageUrl(
        `/avatar/oppia_avatar_100px_${index}.svg`
      );
    } while (this.OPPIA_AVATAR_IMAGE_URL === newUrl);
    this.OPPIA_AVATAR_IMAGE_URL = newUrl;
  }
}
