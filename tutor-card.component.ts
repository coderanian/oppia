...
export class TutorCardComponent {
  ...
  setRandomAvatarImage(): void {
    let index = Math.floor(Math.random() * AppConstants.AVATAR_IMAGE_COUNT + 1);
    let newUrl = this.urlInterpolationService.getStaticCopyrightedImageUrl(
      '/avatar/oppia_avatar_100px_' + index + '.svg'
    );
    if (this.oppia_avatar_image_url === newUrl) {
      this.setRandomAvatarImage();
      return;
    }
